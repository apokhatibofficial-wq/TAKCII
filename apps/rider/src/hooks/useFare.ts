...
import { fareOf, fmtMoney, rowToCamel, type PricingRow, type PricingSettings } from '@takc/shared';
...
export function useFare() {
  ...
  return {
    pricing,
    settings,
    fareFor: (km: number, minutes: number) => (pricing ? fareOf(pricing, km, minutes) : null),
    format: (v: number) => (pricing ? fmtMoney(pricing.currency, v) : '')
  };
}
